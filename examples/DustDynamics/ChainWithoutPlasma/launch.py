...
if __name__ == "__main__":
    ...
    method = "quadratic"
    E_0 = 2683.329450592909
    alpha = 39227.12757274581
    beta = -47197015.80141814
    ...
    n = int(0.5 * tau_p_i/d_t) # Number of ion integration time-step
    ...
    n_dust = 20000 # number of dust particles time steps
    ...
    for iteration in range(1, n_dust):
        ...
        #################################################
        f_io = np.array([9.12368e-13, 7.62704e-13, 6.45765e-13, 6.92236e-13, 4.74270e-13, 5.21961e-13, 4.10071e-13])
        f_d = np.zeros(N_p)
        ...
        for p in range(N_p):
            ...
            f_z[p][iteration] = q[p][0] * (E_0 + alpha*z[p][iteration-1] + beta*z[p][iteration-1]**2)
            f_d[p] = 0
            for _p in range(N_p):
                if p != _p:
                    f_d[p] += q[_p][0]*q[p][0]/(4.0*np.pi*eps_0)*(z[p][iteration-1]-z[_p][iteration-1])/((z[p][iteration-1]-z[_p][iteration-1])**2)**1.5

            _a_x = (f_therm_p_x + f_x[p][iteration] - k_x*x[p][iteration-1]) / m_p
            _a_y = (f_therm_p_y + f_y[p][iteration] - k_y*y[p][iteration-1]) / m_p
            _a_z = (f_therm_p_z + f_z[p][iteration] + f_io[p] + f_d[p]) / m_p + g
        ...
        for p in range(N_p):
            ...
            file.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(t[iteration], x[p][iteration], y[p][iteration], z[p][iteration], v_x[p][iteration], v_y[p][iteration], v_z[p][iteration], f_x[p][iteration], f_y[p][iteration], f_z[p][iteration] + g * m[p], q[p][0]))
            file.close()
