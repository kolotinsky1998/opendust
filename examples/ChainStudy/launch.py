...
if __name__ == "__main__":
    for iteration in range(5,7):
        ...
        file = open("{}/data{}.txt".format(sys.argv[1],iteration-1),"r")
        ...
        for line in file:
            ...
            if readFlag == 1:
                if readIter != 0:
                    _z.append(float(line.replace('\n','').split("\t")[0]))
                    _E_z.append(float(line.replace('\n','').split("\t")[1]))
                    _sigma_E_z.append(float(line.replace('\n','').split("\t")[2]))
                readIter += 1
        ...
        z_max = H/2.0
        E_0, alpha, beta = fit(_z, _E_z, z_min, z_max)
        externalFieldOrder = 2
        v_fl, w_c = velocity(_E_z[0], P)
        M = v_fl/v_B
        ...
        # object for equilibration
        csvOutputFileNameEquilibrate = directory + "csv/trajectoryEquilibrate{}_".format(iteration)
        xyzOutputFileNameEquilibrate = directory + "trajectoryEquilibrate{}.xyz".format(iteration)
        ...
        # object for modelling plasma around chain
        csvOutputFileName = directory + "csv/trajectory{}_".format(iteration)
        xyzOutputFileName = directory + "trajectory{}.xyz".format(iteration)
        restartFileName = directory + "RESTART"
        trapFileName = directory + "trapFile.txt"
        outputParameters = OutputParameters(
            nOutput, nFileOutput, csvOutputFileName, xyzOutputFileName, restartFileName, trapFileName
        )
        ...
        x_1, y_1, z_1, r_1, q_1 = 0, 0, _z[0], r_p, q
        x_2, y_2, z_2, r_2, q_2 = 0, 0, _z[1], r_p, q
        x_3, y_3, z_3, r_3, q_3 = 0, 0, _z[2], r_p, q
        x_4, y_4, z_4, r_4, q_4 = 0, 0, _z[3], r_p, q
        x_5, y_5, z_5, r_5, q_5 = 0, 0, _z[4], r_p, q
        x_6, y_6, z_6, r_6, q_6 = 0, 0, _z[5], r_p, q
        x_7, y_7, z_7, r_7, q_7 = 0, 0, _z[6], r_p, q
